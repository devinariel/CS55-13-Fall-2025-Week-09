...
import { getCliniciansSnapshot } from "@/src/lib/firebase/firestore.js";
import Filters from "@/src/components/Filters.jsx";

const ClinicianItem = ({ clinician }) => (
  <li key={clinician.id}>
    <Link href={`/clinician/${clinician.id}`}>
      <ActiveResturant restaurant={clinician} />
    </Link>
  </li>
);
...
export default function RestaurantListings({
  ...
  useEffect(() => {
    return getCliniciansSnapshot((data) => {
      setRestaurants(data);
    }, filters);
  }, [filters]);
    ...
    <article>
      ...
      <ul className="restaurants">
        {restaurants.map((restaurant) => (
          <ClinicianItem key={restaurant.id} clinician={restaurant} />
        ))}
      </ul>
    </article>
  );
